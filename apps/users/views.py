...
def api(request):
    if request.is_ajax():
        ...
        user_education = UserEducation.objects.filter(user=user_info)
        user_skills = UserSkill.objects.filter(user=user_info)
        user_experience = UserExperience.objects.filter(user=user_info)
